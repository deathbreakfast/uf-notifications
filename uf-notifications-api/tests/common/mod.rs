...
pub async fn seed_user(id: &str, valence: &Valence) {
    ...
    .expect("build user");
    User::upsert(id, user, valence, valence::use_!(r"**Test:** Fixture **User** save for `common` so the suite can arrange and assert persistence behavior. CI and developers running the suite only.")).await.expect("upsert user");
}
